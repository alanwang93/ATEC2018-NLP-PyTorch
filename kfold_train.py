    ...
    for i in range(K):
        ...
        for epoch in range(200):
            ...
            if 'sigmoid' not in args.config:
                ...
                logger.info("Valid at epoch {0}, F1:{1:.4f}, Acc:{2:.3f}, P:{3:.3f}, R:{4:.3f}, Loss:{5:.10f}"\
                ...
                for threshold in np.arange(0.45, 0.85, 0.01):
                    ...
                    logger.info("Valid at epoch {0}, threshold {6}, F1:{1:.4f}, Acc:{2:.3f}, P:{3:.3f}, R:{4:.3f}, Loss:{5:.4f}"\
                            .format(epoch, f1, acc, prec, recall, valid_loss, threshold))
                    f1s.append((f1, threshold))
                print()

            """ Save model """
            model_path = "checkpoints/{0}_{1}_{2}_{3}".format(c['name'], args.suffix, i, K)
